...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize\itshape VolEdge}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{fig/bkm_validation.png}
\caption{BKM extractor versus closed-form truth. On a Black-Scholes chain of
known 20\% volatility, the recovered risk-neutral volatility converges to the
pricing volatility as strike coverage widens; the small residual at narrow
wings is strike truncation, inherent to BKM on any finite chain. This is the
regression test that guards the engine (an earlier version carried a spurious
$1/T$ and doubled $e^{rT}$, inflating moments $\sim$2.7x).}
\label{fig:bkm}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig/garch_volmanaged.png}
\caption{GARCH vol-managed SPY versus buy-and-hold, walk-forward with a 2019
training burn-in so the evaluation window (2020-2024) includes the COVID
crash. The strategy gives up total return for a markedly better path: Sharpe
0.86 versus 0.68, maximum drawdown $-19.7\%$ versus $-33.7\%$. Reproduce with
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig/vix_vrp.png}
\caption{The reality check: the naive variance-premium harvest versus SPY,
walk-forward, net of costs (2019-2024). The COVID-2020 and 2022 volatility
spikes are exactly where the short-vol position pays out its accumulated
premium as crash losses.}
\label{fig:vrp}
\end{figure}
